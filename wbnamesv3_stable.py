...
wbid = ['3', '7', '8', '9', '10', '15', '11', '22', '23', '6', '13', '19']
wb = ['UserAcquisition', 'Engagement', 'Retention', 'Re-Engagement', 'Geography', 'SubLevel', 'LTV', 'TrafficQuality', 'ExecutiveSummary']
...
def update(key1, key, tocName, outfile):
    ...
    try:  # If the id exists, proceed with updating URL
        ...
        if key not in ['6', '13', '19', '22', '23']:
            substitution = "id='%s' is-centered='0' is-scaled='1' param='Image/Blank Space.png' type='bitmap' url='http://bi.tune.com/#/site/%s/workbooks/%s/views?order=name:asc'" % (key, advertiser_name, id)
        elif key == '6':
            ...
            substitution = "id='%s' is-centered='0' is-scaled='1' param='Image/Blank Space.png' type='bitmap' url='http://bi.tune.com/#/site/%s/views?order=name:asc'" % (key, advertiser_name)
        elif key in ['22', '23']:
            substitution = "id='%s' is-centered='0' is-scaled='1' param='Image/Blank Space 1.png' type='bitmap' url='http://bi.tune.com/#/site/%s/workbooks/%s/views?order=name:asc'" % (key, advertiser_name, id)
...
def repack():
    ...
    for name in glob.glob(adv_id + '/*TableOfContents*.twb'):
    ...
    for name in glob.glob(adv_id + '/*TableOfContents*.temp'):
        base_name = os.path.basename(name)
        file_name, file_ext = os.path.splitext(base_name)
    ...
    for name in glob.glob(adv_id + '/*TableOfContents*.zip'):
        base_name = os.path.basename(name)
        file_name, file_ext = os.path.splitext(base_name)
