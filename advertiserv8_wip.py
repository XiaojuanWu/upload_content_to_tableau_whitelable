...
def updateWorkbooks(adv_id, adv_name, from_port, to_port, sharded_schema):
    ...
    for template_file in templates:
        ...
        for name in glob.glob(adv_id + '/*TrafficQualityScorecard*.twb'):
            tqo = open(new_file, "rb")
            tqr = tqo.read()
            tqo.close()
            tqPattern = re.compile(r'bi.tune.com\/#\/site\/(.*)\/views')
            tqSub = tqPattern.sub('bi.tune.com/#/site/' + adv_name + '/views', tqr)
            tqo = open(new_file, 'wb')
            tqo.write(tqSub)
            tqo.close()
            print new_file + " links repointed"
